...
export default function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <main className="auth-shell min-h-dvh bg-[var(--surface-subtle)] lg:grid lg:grid-cols-[minmax(0,0.98fr)_minmax(40rem,1.02fr)]">
      <aside className="auth-showcase relative hidden min-h-dvh overflow-hidden bg-[var(--brand-strong)] px-12 py-10 text-white lg:flex lg:flex-col xl:px-[6.5vw] xl:py-12">
        <TechnicalContours />

        <div className="relative z-10">
          <Image
            alt="Mundo Ar Climatização"
            className="auth-logo h-auto w-48 object-contain xl:w-52"
            height={1254}
            sizes="208px"
        ...
        <div className="auth-showcase-content relative z-10 my-auto max-w-[43rem] py-8">
          ...
          <h1 className="auth-showcase-title font-display mt-7 max-w-[40rem] text-[clamp(2.8rem,3.65vw,4.35rem)] font-bold leading-[1.08] tracking-tight">
            Gestão que mantém
            <br />
            o seu <span className="text-[var(--brand-accent)]">fluxo em movimento.</span>
          </h1>
          <p className="auth-showcase-copy mt-6 max-w-[38rem] text-lg leading-8 text-slate-300">
            Acompanhe atendimentos, diagnósticos e ordens de serviço com
            segurança e eficiência em cada etapa.
          </p>

          <div className="auth-highlights mt-8 grid grid-cols-3 gap-4">
            {highlights.map((highlight) => {
              const Icon = highlight.icon;
              return (
                <article
                  className="auth-highlight-card min-h-44 rounded-xl border border-white/10 bg-white/[0.055] p-5 backdrop-blur-sm"
                  key={highlight.title}
                >
        ...
        <div className="auth-showcase-footer relative z-10 flex items-center gap-3 border-t border-white/10 pt-5 text-sm text-slate-300">
      ...
      <section className="auth-stage relative flex min-h-dvh items-center justify-center overflow-hidden px-4 py-8 sm:px-8 lg:px-12">
        ...
        <div className="auth-card relative w-full max-w-[39.5rem] rounded-2xl border border-[var(--border)] bg-white px-5 py-8 shadow-[0_24px_65px_rgba(16,45,63,0.09)] sm:px-10 sm:py-10 xl:px-14 xl:py-11">
          {children}

          <footer className="auth-card-footer mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-[var(--border)] pt-5 text-xs font-medium text-[var(--ink-muted)]">
