...
const toneClasses = {
  ...
  green: { icon: "bg-green-50 text-green-700", trend: "text-green-700" },
  red: { icon: "bg-red-50 text-red-600", trend: "text-red-600" },
} as const;

const orders = [
  {
    id: "#0148", priority: "Prioridade média", priorityClass: "text-amber-600",
  ...
  {
    id: "#0147", priority: "Prioridade alta", priorityClass: "text-red-600",
  ...
  {
    id: "#0146", priority: "Prioridade baixa", priorityClass: "text-green-700",
  ...
  {
    id: "#0145", priority: "Prioridade média", priorityClass: "text-amber-600",
  ...
  {
    id: "#0144", priority: "Atrasada", priorityClass: "text-red-600",
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="text-[0.68rem] font-bold uppercase leading-4 tracking-[0.04em] text-[var(--ink-muted)]">{metric.label}</p>
          ...
          <p className={cn("mt-2 flex items-center gap-1 text-[0.68rem] font-semibold", colors.trend)}>
...
function StatusBadge({ className, children }: { className: string; children: string }) {
  return (
    <span className={cn("inline-flex max-w-28 items-center gap-1.5 rounded-full px-2.5 py-1 text-[0.65rem] font-bold leading-3 ring-1 ring-inset", className)}>
    ...
    <section className="overflow-hidden rounded-xl border bg-white shadow-[0_2px_8px_rgba(16,45,63,0.04)]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[56rem] border-collapse text-left">
          ...
          <thead>
            <tr className="border-b bg-[#f8fafb] text-[0.62rem] font-bold uppercase tracking-[0.06em] text-[var(--ink-faint)]">
            ...
            {orders.map((order) => (
              <tr className="border-b last:border-b-0 hover:bg-[var(--surface-subtle)]" key={order.id}>
                <td className="px-4 py-2.5">
                  ...
                  <p className={cn("mt-0.5 text-[0.63rem] font-semibold", order.priorityClass)}>{order.priority}</p>
                </td>
                <td className="px-3 py-2.5">
                  <div className="flex items-center gap-2.5">
                    <CarFront aria-hidden="true" className="size-4 shrink-0 text-[var(--ink-faint)]" />
                    <div><p className="text-xs font-semibold text-[var(--ink)]">{order.vehicle}</p><p className="text-[0.65rem] text-[var(--ink-muted)]">{order.plate}</p><p className="text-[0.65rem] text-[var(--ink-muted)]">{order.year}</p></div>
                  </div>
                </td>
                <td className="px-3 py-2.5"><p className="text-xs font-medium text-[var(--ink)]">{order.customer}</p><p className="mt-0.5 text-[0.65rem] text-[var(--ink-muted)]">{order.phone}</p></td>
                <td className="px-3 py-2.5 text-[0.68rem] text-[var(--ink)]"><p>{order.entry}</p><p className="text-[var(--ink-muted)]">{order.entryTime}</p></td>
                <td className={cn("px-3 py-2.5 text-[0.68rem]", order.status === "Atrasada" ? "font-semibold text-red-600" : "text-[var(--ink)]")}><p>{order.due}</p><p>{order.dueTime}</p></td>
...
function QuickStat({ item, index }: { item: (typeof quickStats)[number]; index: number }) {
  ...
  const color = item.tone === "amber" ? "text-amber-600 bg-amber-50" : item.tone === "blue" ? "text-blue-600 bg-blue-50" : "text-green-700 bg-green-50";
  return (
    <article className="rounded-xl border bg-white p-3 shadow-[0_2px_8px_rgba(16,45,63,0.035)]">
      <div className="flex items-start justify-between gap-2">
        <div><p className="text-[0.65rem] font-semibold text-[var(--ink-muted)]">{item.label}</p><p className="mt-1 text-xl font-bold tracking-tight text-[var(--brand)]">{item.value}</p></div>
      ...
      <div className="mt-1.5 flex items-end justify-between gap-2">
        <p className={cn("text-[0.58rem] font-semibold", color.split(" ")[0])}>
  ...
  return (
    <section className="overflow-hidden rounded-xl border bg-white shadow-[0_2px_8px_rgba(16,45,63,0.04)]">
      <div className="flex items-center gap-3 border-b border-red-100 bg-red-50/70 px-4 py-3">
        <TriangleAlert aria-hidden="true" className="size-5 shrink-0 text-red-600" />
        <div className="min-w-0 flex-1"><p className="text-[0.67rem] font-bold uppercase tracking-[0.08em] text-red-600">Ação necessária</p><p className="text-xs font-medium text-[var(--ink)]">Precisam de atenção agora</p></div>
        <span className="rounded-md border border-red-200 bg-white px-2 py-1 text-[0.65rem] font-semibold text-red-600">4 itens</span>
      </div>
      <ol className="divide-y px-4">
        {pendingItems.map((item) => (
          <li className="py-3" key={item.title}>
            <div className="flex gap-3">
              <div className="min-w-0 flex-1"><p className="text-xs font-semibold text-[var(--brand)]">{item.title}</p><p className="mt-0.5 text-[0.67rem] text-[var(--ink-muted)]">{item.detail}</p><button className="mt-1.5 flex items-center gap-1 text-[0.68rem] font-semibold text-[var(--action)]" disabled type="button">{item.action}<ArrowRight aria-hidden="true" className="size-3" /></button></div>
              <span className="shrink-0 text-[0.62rem] font-semibold text-red-600">{item.age}</span>
            </div>
          </li>
        ))}
      </ol>
      <div className="px-4 pb-3"><button className="flex min-h-8 w-full items-center justify-center gap-2 rounded-md border text-[0.68rem] font-medium text-[var(--ink-muted)]" disabled type="button">Ver todas as pendências<ArrowRight aria-hidden="true" className="size-3" /></button></div>
    </section>
  );
}
...
function NextActionsPanel() {
  return (
    <section className="rounded-xl border bg-white p-4 shadow-[0_2px_8px_rgba(16,45,63,0.04)]">
        ...
        {nextActions.map((item) => {
          ...
          return (
            <li key={item.title}><button className="flex min-h-10 w-full items-center gap-2.5 rounded-md border px-2.5 text-left disabled:opacity-100" disabled type="button">
              ...
              <span className="min-w-0 flex-1"><span className="block truncate text-[0.68rem] font-medium text-[var(--brand)]">{item.title}</span><span className="block text-[0.58rem] text-[var(--ink-muted)]">{item.detail}</span></span>
      ...
      <button className="mt-2 flex min-h-8 w-full items-center justify-center gap-2 rounded-md border text-[0.68rem] font-medium text-[var(--ink-muted)]" disabled type="button">Ver todas as tarefas<ArrowRight aria-hidden="true" className="size-3" /></button>
    </section>
  );
}
    ...
    <AppShell currentPath="/" user={user}>
      <div className="min-h-[calc(100dvh-5.35rem)] px-3 py-3 sm:px-5 lg:px-6">
        <div className="mx-auto max-w-[96rem]">
          ...
          <footer className="flex flex-wrap items-center justify-between gap-3 px-0.5 pb-1 pt-5 text-[0.65rem] text-[var(--ink-muted)]">
